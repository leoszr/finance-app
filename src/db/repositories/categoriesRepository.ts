...
export function createCategoriesRepository(database: RepositoryDatabase = getRepositoryDatabase()) {
  return {
    async createCategory(input: CategoryInput): Promise<RepositoryResult<CategoryRecord>> {
      ...
      const category = await this.getCategoryById(result.lastInsertRowId ?? 0);
    ...
    async getCategoriesByType(type: TransactionType): Promise<CategoryRecord[]> {
    ...
    async updateCategory(id: number, input: CategoryInput): Promise<RepositoryResult<CategoryRecord>> {
      const valid = validateCategory(input);
      if (!valid.ok) return valid;

...
export async function generateAndShareReportPdf(report: MonthlyReport, year: number, month: number): Promise<Result<PdfResult>> {
  try {
    const html = buildReportPdfHtml(report, year, month);
